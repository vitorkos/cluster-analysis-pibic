...
img_path = filedialog.askopenfilename(title="Selecione a imagem para análise")
...
if os.path.isfile(img_path) and img_path.lower().endswith(('.png', '.jpg', '.jpeg')):
    ...
    bin_image_path = os.path.splitext(img_path)[0] + '_bin.jpg'
    ...
    areas = []
    csv_file_path = os.path.splitext(img_path)[0] + '_cluster_data.csv'
    ...
    with open(csv_file_path, 'w', newline='') as csv_file:
        ...
        csv_writer.writerow(['Número total de clusters'])
    ...
    plt.axis('off')
    plt.tight_layout()
    bin_contours_image_path = os.path.splitext(img_path)[0] + '_bin_contours.jpg'
    plt.savefig(bin_contours_image_path, bbox_inches='tight', pad_inches=0)
    plt.close()  # Fecha a figura
else:
    print("O arquivo selecionado não é uma imagem ou não existe.")